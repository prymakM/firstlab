...
public class RestGui extends JFrame{
	...
	private Diagram diagramFreeTables = null;

	private Diagram diagramQueueToOrder = null;
	...
	private ChooseRandom rndClient = null;

	private ChooseRandom rndCook = null;

	private ChooseData chooseDataNTables = null;

	private ChooseData chooseDataNCook = null;


	private ChooseData chooseDataNWaiter = null;
	...
	private ChooseRandom rndWaiter;

	public RestGui() throws HeadlessException {
		super();
		initialize();
	}
	...
	private void startTest() {
		...
		getDiagramFreeTables().clear();
		getDiagramQueueToOrder().clear();
		getDiagramQueueOfKitchen().clear();
		Dispatcher dispatcher = new Dispatcher();		
		dispatcher.addDispatcherFinishListener(
				()->getJButtonStart().setEnabled(true));
		IModelFactory factory = (d)-> new RestModel(d, this);
		RestModel model =(RestModel) factory.createModel(dispatcher);
		model.initForTest();
		dispatcher.start();
	}
	...
	private JPanel getJPanelTest() {
		if (jPanelTest == null) {
			...
			gbl_jPanelTest.rowHeights = new int[]{0, 0, 0, 30, 0};
			jPanelTest.setLayout(gbl_jPanelTest);
			jPanelTest.add(getDiagramFreeTables(), gridBagConstraints8);
			jPanelTest.add(getDiagramQueueToOrder(), gridBagConstraints10);
			jPanelTest.add(getDiagramQueueOfKitchen(), gridBagConstraints11);
			...
			.addComponentListener(new java.awt.event.ComponentAdapter() {
				public void componentShown(
					...
					getChooseDataNTables().select(0,0);
					getChooseDataFinishTime().select(0,0);
					getChooseDataNWaiter().select(0,0);
				}
			});
		}
		return jPanelTest;
	...
	private TransProcessManager getTransProcessManager() {
		if (transProcessManager == null) {
			...
			transProcessManager.setFactory((d)-> new RestModel(d, this));
		}
	...
	private JPanel getJPanelModelParameters() {
		if (jPanelModelParameters == null) {
			...
			jPanelModelParameters.add(getRndClient(), null);
			jPanelModelParameters.add(getRndCook(), null);
			jPanelModelParameters.add(getRndWaiter(), null);
			jPanelModelParameters.add(getChooseDataNTables(), null);
			jPanelModelParameters.add(getChooseDataNCook(), null);
			jPanelModelParameters.add(getChooseDataNWaiter(), null);
	...
	/**
	 * This method initializes rndClient	
	 * 	
	 * @return widgets.ChooseRandom	
	 */
	public ChooseRandom getRndClient() {
		if (rndClient == null) {
			rndClient = new ChooseRandom();
			rndClient.setRandom(new Erlang(3,3));
			rndClient.setTitle("Дії клієнта");
			rndClient.setBounds(new Rectangle(3, 21, 231, 52));
		}
		return rndClient;
	}

	public ChooseRandom getRndWaiter() {
		if (rndWaiter == null) {
			rndWaiter = new ChooseRandom();
			rndWaiter.setBounds(new Rectangle(3, 146, 231, 52));
			rndWaiter.setRandom(new Erlang(20, 4));
			rndWaiter.setTitle("Продуктивність офіціанта");
		}
		return rndWaiter;
	}
	/**
	 * This method initializes rndCook	
	 * 	
	 * @return widgets.ChooseRandom	
	 */
	public ChooseRandom getRndCook() {
		if (rndCook == null) {
			rndCook = new ChooseRandom();
			rndCook.setRandom(new Norm(1,0.2));
			rndCook.setTitle("Продуктивність кухара");
			rndCook.setBounds(new Rectangle(3, 81, 231, 52));
		}
		return rndCook;
	}
	...
	/**
	 * This method initializes chooseDataNTables	
	 * 	
	 * @return widgets.ChooseData	
	 */
	public ChooseData getChooseDataNTables() {
		if (chooseDataNTables == null) {
			chooseDataNTables = new ChooseData();
			chooseDataNTables.setBounds(new Rectangle(3, 200, 231, 53));
			chooseDataNTables.setTitle("Кількість столів");
			chooseDataNTables.setText("12");
			chooseDataNTables.addCaretListener(new javax.swing.event.CaretListener() {
				public void caretUpdate(javax.swing.event.CaretEvent e) {
					if (getJPanelTest().isShowing())
						getDiagramFreeTables().setVerticalMaxText(
								chooseDataNTables.getText());
				}
			});
		}
		return chooseDataNTables;
	}


	/**
	 * This method initializes chooseDataNCook	
	 * 	
	 * @return widgets.ChooseData	
	 */
	public ChooseData getChooseDataNCook() {
		if (chooseDataNCook == null) {
			chooseDataNCook = new ChooseData();
			chooseDataNCook.setBounds(new Rectangle(4, 263, 231, 53));
			chooseDataNCook.setTitle("Кількість кухарів");
			chooseDataNCook.setText("2");
		}
		return chooseDataNCook;
	}
	...
	/**
	 * This method initializes chooseDataNWaiter	
	 * 	
	 * @return widgets.ChooseData	
	 */
	public ChooseData getChooseDataNWaiter() {
		if (chooseDataNWaiter == null) {
			chooseDataNWaiter = new ChooseData();
			chooseDataNWaiter.setBounds(new Rectangle(1, 326, 231, 53));
			chooseDataNWaiter.setTitle("Кількість офіціантів");
			chooseDataNWaiter.setText("3");
		}
		return chooseDataNWaiter;
	}
	...
	public ChooseData getChooseDataFinishTime() {
		if (chooseDataFinishTime == null) {
			...
			.addCaretListener(new javax.swing.event.CaretListener() {
				public void caretUpdate(javax.swing.event.CaretEvent e) {
					if (getJPanelTest().isShowing()) {
						getDiagramFreeTables().setHorizontalMaxText(
								chooseDataFinishTime.getText());
						getDiagramQueueToOrder().setHorizontalMaxText(
								chooseDataFinishTime.getText());
						getDiagramQueueOfKitchen().setHorizontalMaxText(
								chooseDataFinishTime.getText());
					}
				}
			});
	...
	public static void main(String[] args) {
		RestGui application = new RestGui();
	...
	private void initialize() {
		...
		this
				.setTitle("Дослідження роботи ресторану шляхом імітаційного моделювання");
	}
	...
	/**
	 * This method initializes diagramFreeTables
	 * 
	 * @see buldo2011.IBuldoGUI
	 * @return paint.Diagram
	 */
	public Diagram getDiagramFreeTables() {
		if (diagramFreeTables == null) {
			diagramFreeTables = new Diagram();
			diagramFreeTables.setHorizontalMaxText("500");
			diagramFreeTables.setTitleText("Вільні столики");
			diagramFreeTables.setVerticalMaxText("12");
			diagramFreeTables.setPainterColor(new java.awt.Color(204, 102, 0));
		}
		return diagramFreeTables;
	}

	/**
	 * This method initializes diagramQueueToOrder
	 * 
	 * @see buldo2011.IBuldoGUI
	 * @return paint.Diagram
	 */
	public Diagram getDiagramQueueToOrder() {
		if (diagramQueueToOrder == null) {
			diagramQueueToOrder = new Diagram();
			diagramQueueToOrder.setHorizontalMaxText("500");
			diagramQueueToOrder.setTitleText("Черга на обслуговування");
			diagramQueueToOrder.setVerticalMaxText("6");
			diagramQueueToOrder.setPainterColor(java.awt.Color.magenta);
		}
		return diagramQueueToOrder;
	}
	...
	private StatisticsManager getStatisticsManager() {
		if (statisticsManager == null) {
			...
			statisticsManager.setFactory((d)-> new RestModel(d, this));
		}
	...
	private JPanel getJPanelRegres() {
		if (jPanelRegres == null) {
			...
			em.getChooseDataFactors().setTitle("Кількість офіціантів");
			...
			em.setFactory((d)-> new RestModel(d, this));
